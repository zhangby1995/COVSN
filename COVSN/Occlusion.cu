...
extern "C" void occlusion(float *f, int num, vector<int> &result)
{
	...
	/*checkCudaErrors(cudaMalloc((void**) &gpuf, sizeof(float)*9*NUM));*/
	cudaMalloc((void**)&gpuf, sizeof(float)* 9 * num);

	//checkCudaErrors(cudaMalloc((void**) &gpu_yourmother, sizeof(float)*32*NUM));
	/* checkCudaErrors(cudaMalloc((void**) &gpuG2, sizeof(float)*6*NUM*NUM));*/

	/*checkCudaErrors(cudaMalloc((void**) &gpuresult, sizeof(int)*NUM*NUM));*/
	...
	/* checkCudaErrors(cudaMemcpy(FF, gpuresult, sizeof(int)*num*num, cudaMemcpyDeviceToHost));*/
	cudaMemcpy(FF, gpuresult, sizeof(int)*num*num, cudaMemcpyDeviceToHost);

	/*checkCudaErrors(cudaMemcpy(cpuG2, gpuG2, sizeof(float)*6*num*num, cudaMemcpyDeviceToHost));*/
	//checkCudaErrors(cudaMemcpy(cpu_yourmother, gpu_yourmother, sizeof(float)*32*num, cudaMemcpyDeviceToHost));
	cudaFree(gpuf);
	/*cudaFree(gpuG2);*/
	cudaFree(gpuresult);
	//cudaFree(gpu_yourmother);
	for (int s = 0; s<num; s++)
	{
		for (int t = 0; t<num; t++)
		{
